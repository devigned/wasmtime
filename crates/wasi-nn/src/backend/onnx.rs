...
impl BackendExecutionContext for ONNXExecutionContext<'_> {
    ...
    fn compute(&mut self) -> Result<(), BackendError> {
        let shaped_inputs = self
            ...
            .map(|(i, _o)| {
                ...
                match input.tensor_type {
                    TensorType::Fp32 => {
                        let data =
                            unsafe { from_raw_parts(input.data.as_ptr(), input.data.len() * 4) };
                        Array::from_shape_vec(dims, data.to_vec()).unwrap()
                    }
                    TensorType::Fp16 => {
                    ...
                    TensorType::U8 => Array::from_shape_vec(dims, input.data.to_vec()).unwrap(),
                    TensorType::I32 => {
                        let data =
                            unsafe { from_raw_parts(input.data.as_ptr(), input.data.len() * 4) };
                        Array::from_shape_vec(dims, data.to_vec()).unwrap()
        ...
        let res = session.run(shaped_inputs)?;

        for i in 0..self.outputs.len() {
            self.outputs[i].replace(res[i].to_slice().unwrap().to_vec());
        }
        Ok(())
    }
    ...
    fn get_output(&mut self, index: u32, destination: &mut [u8]) -> Result<u32, BackendError> {
        ...
        destination[..output.len()].copy_from_slice(output.as_slice());
